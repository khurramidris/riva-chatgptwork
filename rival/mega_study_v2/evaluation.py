"""Corrected scoring of v1 inputs. This report is a v2 reanalysis, not v1 output."""
...
from .outcomes import outcome_type, extract_outcome_cells, parse_model_json

from ..mega_study.constants import EXPECTED_OUTCOMES, SCHEMA_VERSION, VARIANT_LABELS, VARIANTS
from .resources import load_manifest
from .resources import load_prediction_stage
from ..mega_study.utils import ProtocolError, ResponseParseError, canonical_hash, file_hash, read_jsonl, text_hash
...
def _metric_block(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    if not (np.all(np.isfinite(human)) and np.all(np.isfinite(predicted))
            and np.all(np.isfinite(ranges)) and np.all(ranges > 0)):
        raise ProtocolError("valid metric cells must be finite with positive ranges")
    ...
    types = set(valid["outcome_type"]) if "outcome_type" in valid else {"continuous"}
    if len(types) != 1:
        raise ProtocolError("metric block mixes outcome types")
    kind = next(iter(types))
    result["outcome_type"] = kind
    # Classification scores apply only to explicitly declared ordinal outcomes.
    if kind == "ordinal":
        classes = np.unique(human)
        if len(classes) >= 2:
            result["balanced_accuracy"] = float(np.mean([
                np.mean(predicted[human == label] == label) for label in classes
            ]))
    elif kind not in {"composite", "continuous"}:
        raise ProtocolError(f"unsupported metric outcome type {kind!r}")
    if kind == "continuous":
        result["exact_accuracy"] = None
        result["distribution_tvd"] = None
    return result
...
def _paired_lift(
    ...
    *,
    ...
    if len(pids) < 2:
        return {"treatment": treatment, "baseline": baseline, "study_id": study or "macro",
                "paired_cells": len(paired), "paired_participants": len(pids), "mean_lift": point,
                "ci95_lower": None, "ci95_upper": None,
                "interval_status": "INSUFFICIENT_INDEPENDENT_PARTICIPANTS"}
...
def _cell_frame(
    ...
    for case in cases:
        ...
        for variant in VARIANTS:
            ...
            # Never trust a historical SUCCESS flag or already-averaged cells.
            # Validate the preserved raw response using the answer-free instrument.
            result = dict(result)
            if result.get("status") == "SUCCESS":
                try:
                    raw = result["raw_response"]
                    if text_hash(raw) != result.get("raw_response_sha256"):
                        raise ResponseParseError("raw response hash is missing or invalid")
                    revised = extract_outcome_cells(study, case["survey_text"], parse_model_json(raw))
                    result["predicted_cells"] = [cell.as_dict() for cell in revised]
                except (KeyError, TypeError, ResponseParseError):
                    result["status"] = "REVALIDATION_FAILURE"
                    result["predicted_cells"] = []
            predicted = {
                ...
                rows.append(
                    {
                        ...
                        "outcome_type": outcome_type(study, outcome_id),
...
def evaluate_benchmark(
    ...
    if marker.get("runtime_revision") == RUNTIME_REVISION:
        ...
        if not journal_path.is_file() or file_hash(journal_path) != marker.get("attempt_journal_sha256"):
            raise ProtocolError("attempt journal changed or is missing after v2 freeze")
    ...
    return {
        ...
        "status": "COMPLETE_REANALYSIS_REPORT",
        "runtime_revision": RUNTIME_REVISION,
        "analysis_role": "corrected development reanalysis; not a frozen v1 report",
        "source_results_sha256": file_hash(results),
        "classification_policy": "balanced accuracy only for declared ordinal outcomes",
        "raw_response_policy": "every historical success revalidated against answer-free instrument",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    def lift_fmt(value: dict[str, Any]) -> str:
        if value["mean_lift"] is None:
            return "—"
        if value["ci95_lower"] is None or value["ci95_upper"] is None:
            return f"{value['mean_lift']:.4f} (interval unavailable)"
        return (
    ...
    lines = [
        "# Rival Mega-Study v2 Corrected Development Reanalysis",
        "",
