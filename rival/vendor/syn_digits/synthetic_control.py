...
# RIVAL PORTABILITY PATCH (2026-08-28): upstream imports
# ``causaltensor.matlib.SVD``. The upstream operation is a rank-k truncated
# reconstruction, so keep the call shape with NumPy.
def SVD(matrix: np.ndarray, rank: int) -> np.ndarray:
    values = np.asarray(matrix, dtype=float)
    u, singular_values, vt = np.linalg.svd(values, full_matrices=False)
    selected = max(1, min(int(rank), len(singular_values)))
    return (u[:, :selected] * singular_values[:selected]) @ vt[:selected, :]

OUTPUT_DIR = os.environ.get(
    "RIVAL_SYN_DIGITS_OUTPUT_DIR",
    os.path.join(tempfile.gettempdir(), "rival-syn-digits"),
)
