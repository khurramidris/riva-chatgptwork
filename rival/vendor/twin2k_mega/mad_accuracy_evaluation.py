...
def summary_mad(values) -> Tuple[float, float, float, float]:
    ...
    ci_low, ci_high = (
        t.interval(0.95, len(arr) - 1, loc=mean_mad, scale=stderr)
        if len(arr) > 1
        else (np.nan, np.nan)
    )
...
def compute_column_mad(
    ...
    random_baseline: Optional[pd.DataFrame] = None,
) -> pd.DataFrame:
    ...
    common_cols = list(
        set(predictions.columns) & set(ground_truth.columns) & set(column_ranges.keys())
    )
            ...
            row.update(
                {
                    ...
                    "predictions accuracy / random accuracy": (
                        (1 - norm_diff_pred.mean()) / (1 - norm_diff_rand.mean())
                        if (1 - norm_diff_rand.mean()) != 0
                        else np.inf
                    ),
                }
            )
                ...
                row.update(
                    {
                        ...
                        "random_baseline vs. ground_truth Accuracy 95% CI Higher": ci_hi_rand,
                        "predictions accuracy / random accuracy": (
                            (1 - col_diffs_pred.mean()) / (1 - col_diffs_rand.mean())
                            if (1 - col_diffs_rand.mean()) != 0
                            else np.inf
                        ),
                    }
                )
...
def get_default_qid_to_task() -> Dict[str, str]:
    ...
    qid_to_task_raw = {
        ...
        **{
            f"nonseparabilty bene _{i}": "non-separability of risks and benefits"
            for i in range(1, 5)
        },
        **{
            f"nonseparability ris _{i}": "non-separability of risks and benefits"
            for i in range(1, 5)
        },
...
def compute_mad_summary(csv_dir: str, output_excel_path: str, output_fig_path: str, fig_title: str):
    ...
    try:
        ...
        df_wave1_3 = pd.read_csv(
            os.path.join(csv_dir, "responses_wave1_3_formatted.csv"), skiprows=[1]
        )
        df_wave4 = pd.read_csv(os.path.join(csv_dir, "responses_wave4_formatted.csv"), skiprows=[1])
        df_llm = pd.read_csv(
            os.path.join(csv_dir, "responses_llm_imputed_formatted.csv"), skiprows=[1]
        )
    ...
    logger.debug(
        f"Loaded data shapes: wave1_3: {df_wave1_3.shape}, wave4: {df_wave4.shape}, llm: {df_llm.shape}"
    )
    ...
    for df in [df_wave1_3, df_wave4, df_llm]:
        if "QID164_GROUP" in grouped_deciles:
            for col in existing_decile_cols_164:
                if col in df.columns:
                    df[col] = df[col].apply(
                        lambda x: assign_decile(x, grouped_deciles["QID164_GROUP"])
                    )
        if "QID168_GROUP" in grouped_deciles:
            for col in existing_decile_cols_168:
                if col in df.columns:
                    df[col] = df[col].apply(
                        lambda x: assign_decile(x, grouped_deciles["QID168_GROUP"])
                    )

    cols_all = [
        col for col in df_wave1_3.columns if col in df_wave4.columns and col in df_llm.columns
    ]
    ...
    if len(cols_all) == 0:
        ...
        logger.warning(
            "This might indicate data compatibility issues between the expected and actual column names."
        )
        ...
        with pd.ExcelWriter(output_excel_path, engine="openpyxl") as writer:
            mad_task_summary_df.to_excel(
                writer, sheet_name="Accuracy Summary - task level", index=False
            )
    ...
    # Merge the results
    mad_col_df = mad_col_wave4.rename(
        columns={
    ...
    # Add LLM columns
    llm_cols = mad_col_llm[
        [
            "Column",
            "predictions vs. ground_truth",
            "predictions vs. ground_truth Accuracy",
            "predictions vs. ground_truth Accuracy 95% CI Lower",
            "predictions vs. ground_truth Accuracy 95% CI Higher",
        ]
    ]
    llm_cols.columns = [
        "Column",
        "llm vs. wave1_3",
        "llm vs. wave1_3 Accuracy",
        "llm vs. wave1_3 Accuracy 95% CI Lower",
        "llm vs. wave1_3 Accuracy 95% CI Higher",
    ]
    mad_col_df = mad_col_df.merge(llm_cols, on="Column")

    # Add llm accuracy / wave4 accuracy
    mad_col_df["llm accuracy / wave4 accuracy"] = (1 - mad_col_df["llm vs. wave1_3"]) / (
        1 - mad_col_df["wave4 vs. wave1_3"]
    )

    # Add random baseline columns
    rand_cols = mad_col_rand[
        [
            "Column",
            "predictions vs. ground_truth",
            "predictions vs. ground_truth Accuracy",
    ...
    rand_cols.columns = [
        ...
        "random_baseline vs wave1_3 Accuracy",
        "random_baseline vs. wave1_3 Accuracy 95% CI Lower",
        "random_baseline vs. wave1_3 Accuracy 95% CI Higher",
    ]
    mad_col_df = mad_col_df.merge(rand_cols, on="Column")

    # Add random_baseline accuracy / wave4 accuracy
    mad_col_df["random_baseline accuracy / wave4 accuracy"] = (
        1 - mad_col_df["random_baseline vs wave1_3"]
    ) / (1 - mad_col_df["wave4 vs. wave1_3"])
    ...
    # Merge the results
    mad_task_level_df = mad_task_wave4.rename(
        columns={
            "predictions vs. ground_truth": "wave4 vs. wave1_3",
            "predictions vs. ground_truth Accuracy": "wave4 vs. wave1_3 Accuracy",
            "predictions vs. ground_truth Accuracy 95% CI Lower": "wave4 vs. wave1_3 Accuracy 95% CI Lower",
    ...
    # Add llm accuracy / wave4 accuracy
    mad_task_level_df["llm accuracy / wave4 accuracy"] = (
        1 - mad_task_level_df["llm vs. wave1_3"]
    ) / (1 - mad_task_level_df["wave4 vs. wave1_3"])

    # Add random baseline columns
    rand_task_cols = mad_task_rand[
        [
            "Task",
            "predictions vs. ground_truth",
            "predictions vs. ground_truth Accuracy",
    ...
    # Add random_baseline accuracy / wave4 accuracy
    mad_task_level_df["random_baseline accuracy / wave4 accuracy"] = (
        1 - mad_task_level_df["random_baseline vs wave1_3"]
    ) / (1 - mad_task_level_df["wave4 vs. wave1_3"])
    ...
    for rid in df_wave1_3.index:
        ...
        for col in cols_all:
            ...
            if not task:
                continue
            if (
                pd.notna(df_wave1_3.at[rid, col])
                and pd.notna(df_wave4.at[rid, col])
                and pd.notna(df_llm.at[rid, col])
            ):
    ...
    labels = ["Test-retest", "Digital twins", "Random"]
    ...
    for off, mean, err, lbl, col in zip(offsets, means, errs, labels, colors, strict=False):
        ax.barh(x + off, mean, height=bar_h, color=col, xerr=err, capsize=5, label=lbl)
    ...
    # average lines
    for a, lbl, col in zip(avg, labels, colors, strict=False):
        ax.axvline(a, color=col, ls="--", lw=1.5, label=f"{lbl} - average = {a:.2f}%")
...
def main():
    ...
    parser.add_argument(
        "--output-excel-filename", default="mad_accuracy_summary.xlsx", help="Output Excel filename"
    )
    parser.add_argument(
        "--output-plot-filename", default="accuracy_dist.png", help="Output plot filename"
    )
    ...
    if args.config:
        ...
        output_dir_template = config.get("evaluation", {}).get(
            "output_dir", f"{trial_dir}/accuracy_evaluation"
        )
        output_dir = (
            output_dir_template.replace("${trial_dir}", trial_dir)
            if trial_dir
            else "accuracy_evaluation"
        )
        ...
        title = (
            args.plot_title.replace("LLM", model_name)
            if "LLM" in args.plot_title
            else args.plot_title
        )
    ...
    # Handle legacy positional arguments
        ...
        parser.error(
            "Either --config must be provided, or --csv-dir and --output-dir, or legacy positional arguments"
        )
